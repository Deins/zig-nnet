...
pub fn forNet(comptime NNet: anytype) type {
    return struct {
            ...
            main_worker_loop: while (true) {
                ...
                if (input_idx < inputs.len) {
                    ...
                    if (r.test_cases > 0) {
                        ...
                        var total_i: u32 = @intFromFloat(total);
                        wq.results_done.store(total_i, .Release);
                        if (total_i == inputs.len) {
        ...
        pub fn trainEpoches(self: *Self, net: *NNet, test_accesor: *TestAccessor, epoches: u32) !void {
            ...
            for (shuffled, 0..) |*e, idx| { e.* = @intCast(idx); }
            ...
            while (epoch_i < epoches) : (epoch_i += 1) {
                ...
                while (bb < test_len) : (bb += self.batch_size) {
                    ...
                    wq.inputs = batch;
                    wq.next_input.store(@intCast(0), .Release);
                    ...
                    if (batch.len != wq.results.test_cases)
                    ...
                    {
                        print_timer.reset();
                        const acc : Float= 100 * @as(Float, @floatFromInt(wq.results.correct)) / @as(Float, @floatFromInt( batch.len));
