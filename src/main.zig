...
const NNet = struct {
        ...
        { // Hidden layer
...

            {
                ...
                for (self.w1, 0..) |_, nidx| {
                    // how much total error change with respect to the weights:
                    //      𝝏total_err / 𝝏w
                    const d_err_w = d_tmp * @as(@Vector(sizes[3], Float), @splat(d_o_w[nidx]));
    ...
    pub fn learn(self: *Self, train_results: TrainResult, learn_rate: Float) void {
        @setFloatMode(.Optimized);
        self.bo += train_results.d_bo * @as(@Vector(sizes[3], Float), @splat(learn_rate));
        self.b2 += train_results.d_b2 * @as(@Vector(sizes[3], Float), @splat(learn_rate));
        for (self.w1, 0..) |*w, nidx| {
            w.* -= train_results.d_w1[nidx] * @as(@typeInfo(@TypeOf(w.*)), @splat(learn_rate));
        }
        for (self.w2, 0..) |*w, nidx| {
            w.* -= train_results.d_w2[nidx] * @as(@typeInfo(@TypeOf(w.*)), @splat(learn_rate));
        }
    }
};
...
pub fn train(alloc: mem.Allocator) !void {
    ...
    const seed = 364123;
    var rand_gen = std.rand.Sfc64.init(seed);
    var rnd: std.rand.Random = rand_gen.random();
    ...
    // train
    var timer = try std.time.Timer.start();
    try trainer.trainEpoches(&net, &td.accessor, @intCast(options.epoches));
...
pub fn main() !void {
    ...
    { // ARGS
        ...
        for (args, 0..) |argv, ai| {
            ...
            if (std.ascii.startsWithIgnoreCase(argv, "--workers")) {
                ...
                skip = 1;
            } else if (std.ascii.startsWithIgnoreCase(argv, "--epoches")) {
                if (ai + 1 >= args.len) std.debug.panic("Argument '{s}' needs to be followed by value!", .{argv});
                options.epoches = try std.fmt.parseUnsigned(usize, args[ai + 1], 0);
                skip = 1;
            } else if (std.ascii.startsWithIgnoreCase(argv, "--learn-rate")) {
                if (ai + 1 >= args.len) std.debug.panic("Argument '{s}' needs to be followed by value!", .{argv});
                options.learn_rate = try std.fmt.parseFloat(Float, args[ai + 1]);
                skip = 1;
            } else if (std.ascii.startsWithIgnoreCase(argv, "--load")) {
                if (ai + 1 >= args.len) std.debug.panic("Argument '{s}' needs to be followed by path!", .{argv});
                options.load = args[ai + 1];
                skip = 1;
            } else if (std.ascii.startsWithIgnoreCase(argv, "--save")) {
                if (ai + 1 >= args.len) std.debug.panic("Argument '{s}' needs to be followed by path!", .{argv});
                options.save = args[ai + 1];
                skip = 1;
            } else if (std.ascii.startsWithIgnoreCase(argv, "--batch-size")) {
                if (ai + 1 >= args.len) std.debug.panic("Argument '{s}' needs to be followed by path!", .{argv});
                ...
                options.epoches = try std.fmt.parseUnsigned(usize, args[ai + 1], 0);
                skip = 1;
            } else if (std.ascii.startsWithIgnoreCase(argv, "--img-dir-out")) {
                if (ai + 1 >= args.len) std.debug.panic("Argument '{s}' needs to be followed by path!", .{argv});
                options.img_dir_out = args[ai + 1];
                skip = 1;
            } else if (std.ascii.startsWithIgnoreCase(argv, "preprocess")) {
                {
            ...
            } else if (std.ascii.startsWithIgnoreCase(argv, "train")) {
                train(alloc) catch |err| debug.panic("Error: {}", .{err});
            } else if (std.ascii.startsWithIgnoreCase(argv, "test")) {
                try doTest(alloc);
            } else if (std.ascii.startsWithIgnoreCase(argv, "help")) {
                printHelp();
